...
* Eliminating one occurrence of county = 193, which is duplicated. Also happens to be the last row.

* Reassigning the indexes to county number.
...
Clearly, if we apply log transformation on crime rate, our distribution becomes more normally distributed with mean and median to be very close, almost no skew and symmetric. This log transformed crime rate could be more ideal when it comes to modelling for OLS.
...
First scatter plot matrix is crime rate with variables related to the nature of crime: probabilities of arrest, conviction and prison sentence, average sentence days, and log transformation of offense mix. 
...
Additionally, it is interesting to point that probability of arrest (`prbarr`) and probability of conviction (`prbcanv`) are not highly correlated as we could expect from common sense. This indicates that keeping the two variables in an analysis will not weaken our model due to multicolinearity, but further investigation will be necessary.
...
cor(crime_data$prbarr, crime_data$prbconv, use="complete.obs")
```
...
Second scatter plot matrix is crime rate with variables related to population: police per capita, people per square mile, % minority, and log transformation of % young male.

Here are some features noticed from the matrix:

* There are noticeable positive relationship between crime rate and police per capita, crime rate and people per sq. mi., % young male and crime rate.

* Positive relationship between crime rate and police per capita seems to be an anomaly since crime rate is supposed to go down if there is more police per capita. Therefore, $polpc$ could be a top-coded variable with data not reflected with appropriate variable name. This could also be explained by local governs increasing police presence in higher crime rate areas as an attempt to reduce crimes. If this is true, however, we can see that increasing police only cannot reduce crime rate.
...
Third scatter plot matrix is crime rate with variables related to wages: tax revenue per capita, weekly wages of 6 different industries, and wages of federal, state, and local employees.
...
```{r}
scatterplotMatrix(~ crmrte + taxpc + wcon + wtuc + wtrd + wfir 
...
The covariates that help us further identify a causal effect are $prbarr$ and $prbconv$, $density$ and $pctymle$ based on output from scatter plots. On the other hand, the problematic covariates due to multicollinearity are $taxpc$ and $w*$ (all wages variables) seen from the scatter plot above since they will absorb some of causal effect we want to measure.
...
Picking variables which are only applicable for policy suggestions as the key interest with no other covariates from each variable. As discussed earlier, we decided to keep probabilities of conviction and arrest in our model, since they are not highly correlated (as common sense could infer).

```{r}
rownames(crime_data) <- crime_data$county
...
This model suggests that points 55, 113, and 53 have high Cook's distance, so further investigations were made to define why they are outliers comparing to the expectations of the variables for all other counties:
...
We see that no major deviations are found. When analyzing county 55, we see that this is an area with considerable higher crime rates, but with all variables not very different from the others. Based on the lower populational density and the wages we can infer this is a rural county, which economy is based in construction and transportation industries. The highlight here is the `taxpc` variable: it is more than the 3 times the state average. This discrepancy probably is generating our leverage, but as we don't have enough evidence that this is a erroneous error, we will keep that point in the data set.

The counties represented by 53 and 113 are the opposite. They have crime rates as low as 40% of the state average, small populational density and no assumptions can be made based only in the wages per industry. As we did with 55, we found no strong evidence that this data is wrong, thus, keeping the data in our data set.

It is interesting to note that the three identified outliers have completely different values for `pctmin80`, yet still similar values for most variables. This endorses our decision to not use that variable.
...
Adjusted R^2^ increases by 9.6% by adding one additional variable, and AIC decreases by 2.1% to indicate improvements on parsimony. However, there is not a significant improvement as the solid red line still getting very close to the danger zone of Cook's distance.

Note by the coefficients that the `prbconv` variable is the only without statistical significance in this test, so the model was rebuild to remove that variable. As stated earlier this variable is not correlated with `prbarr`, however it can have relationship with other (even omitted ones that were lumped under the error term and are not available for analysis). 
...
As expected, the `pctymle` variable is substantially higher that the state average, however there are no evidences that there is an error in our data, so the observation will still be used in our data set.
...
Adjusted R^2^ increases by 34% by adding 2 additional variables, and AIC decreases by 28.4% to indicate further improvements on parsimony. Moreover, there is a significant improvement since the solid red line moves away from the danger zone of Cook's distance.
...
We can assume that this model have linear coefficients only because we have not constrained our error term, so the assumption that the $u$ term will incorporate non-linearities is true, and thus the model is linear.

2. Random sampling

While background data was not provided for this analysis, we notice that our sample have 80 different counties. A quick search on North Carolina website shows 100 counties in that state, with the youngest one created in 1911 and none incorporated by other since than. Under this fact, and the assumption that the numbers are official from each county, we can assume that we have analyzed data referent to 80% of the population, being enough to reduce the non-random sampling effect to minimum.

3. No perfect multicollinearity

As R did not warned for any perfect multicolinearity, this assumption is met. Additionally we visually checked for that using `scatterplotMatrix` and  the correlation index.
...
As evidenced in Residuals vs Fitted values plot above, there is no clear deviation from zero conditional mean indicated by the red line. Therefore we can consider the zero-conditional assumption met.
...
We have a sample size > 30, so the CLR tells us that our estimators will have a normal sampling distribution. We might also consider the formal Shapiro-Wilk test of normality. The null hypothesis is the residuals are normally distributed. p-value indicates it can't be rejected, meaning residuals are close enough to normal distribution.
```{r}
...
### 6. Other ommited variables

While our data set have 25 variables, we noticed that more socio-economic and infra-structure variables could improve our model. Examples of extra variables that could be added, and the theories we could test with them, are:
* Educational degree of population (better skilled residents may commit less crime);
...
* Increase the probability of arrest when offense occurs. This not necessarily means increasing the number of police officers in the streets, as seen in our analysis. This change could be addressed with programs that incentive crime reporting practices and population confidence in the law enforcement. Our best model suggests that an improve of 1 percentual point in arresting people that committed crimes may improve crime rate in 2%.

* Decrease the tax revenue per capita by reducing local tax rate. Less taxes means more money in counties' economy, so it may be a way to improve earnings by the population and therefore decrease criminality. The effect, however may not be really big: reducing by 1 percentual point in the tax revenue per capta might impact crime rate reducing it approximately 0.13%. In other words, this variable may have a high statistical importance, but not a practical one.

* Decrease the percentage of young male people in communities. While this can turn to a highly unethical advice, we can try to address this matter with making other areas attractive for young male population using government fostered jobs, for example.

As further developments is this analysis, we may work on our data to decouple the `density` variable and use it, once it was very promising in the tests made during this report.
