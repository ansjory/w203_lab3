...
Additionally, it is interesting to the point that probabilty of arrest $prbarr$ and probability of convicition $prbconv$ are not highly correlated as we could expect from common sense. This indicates that keeping the two variables in an analysis will  weaken our model due to multicolinearity, but furthter investigation will be necessary.
```{r}
...
As R didn't warn for any perfect collinearity, this assumption is met. Additionally we visually checked for that using \texttt{scatterplotMatrix} and the correlation index.
